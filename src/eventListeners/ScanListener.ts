...
let msgNoFace: string = MSG_ERR_NO_FACE;
let msgInvalidImage: string = MSG_ERR_INVALID_IMAGE;
let msgInitializing: string = MSG_FACESCAN_INITIALIZING;
let msgScanning: string = NAME_SCANNING;
let msgScanningStatus: string = MSG_INFO_SCANNING_STATUS;

let productsIsReady: boolean = false;
let faceIsReady: boolean = false;
...
export const ScanListener = (): void => {
  ...
  if (element) {
    ...
    msgNoFace = element.getAttribute('data-noface') || MSG_ERR_NO_FACE;
    msgInvalidImage = element.getAttribute('data-invalid') || MSG_ERR_INVALID_IMAGE;
    msgInitializing = element.getAttribute('data-initializing') || MSG_FACESCAN_INITIALIZING;
    msgScanning = element.getAttribute('data-scanning') || NAME_SCANNING;
    msgScanningStatus = element.getAttribute('data-scanning-status') || MSG_INFO_SCANNING_STATUS;
    ...
    loadFaceModels(options).then(() => {
      ...
      const resultElement = document.getElementById(EL_ID_RESULT_CONTAINER) as HTMLElement;
      if (!resultElement || productsIsReady) {
...
export const updateScannerStatusReady = (): void => {
